...
// The static base (everything in buildSystemPrompt before the per-user tail) is
// the same across all users and sessions, so we cache it with Anthropic's
// prompt-caching API.  The dynamic tail (user profile + animal context + mode +
// frequency layer) changes per user/session and is NOT cached.
...
/**
 * Returns the system prompt split into a static part (cacheable, identical
 * across all users and sessions) and a dynamic part (per-user/session, not cached).
 * Use this when building the Anthropic request body — pass the two pieces as
 * separate text blocks with cache_control on the first.
 */
